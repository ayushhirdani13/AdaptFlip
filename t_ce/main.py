...
def parse_args():
    ...
    parser.add_argument("--batch_size",
        type=int,
        default=1024,
        help="batch size for training (group_size if batch_mode is neighbor), default: 1024",)
...
def test(model, test_data_pos, user_pos):
    ...
    # print(f"################### TEST ######################")
    testres = pd.DataFrame(test_results, index=[f"@{i}" for i in args.top_k]).round(4).head()
...
if __name__ == "__main__":
    ...
    MODEL_FILE = f"{args.model}_{args.drop_rate}_{args.num_gradual}_{args.batch_size}@{args.best_k}.pth"
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch[{epoch+1:03d}/{args.epochs:03d}], Train Loss: {epoch_loss:.4f}", f"Epoch[{epoch+1:03d}/{args.epochs:03d}], Eval Loss: {eval_loss:.4f}")
    ...
    if args.out == True:
        results_df.to_csv(os.path.join(RESULT_DIR, f"{args.model}_{args.drop_rate}_{args.num_gradual}_{args.batch_size}@{args.best_k}.csv"), index=False, float_format="%.4f")
