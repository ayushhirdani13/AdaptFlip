...
def test(model, test_data_pos, user_pos):
    ...
    #     print(f"{k}: {v:.4f}")
    print(pd.DataFrame(test_results, index=[f"@{i}" for i in args.top_k]).round(4).head(len(top_k)))
    print("################### TEST END ######################\n")
...
if __name__ == "__main__":
    ...
    MODEL_DIR = f"models/{DATASET}/item_wise"
    RESULT_DIR = f"results/{DATASET}/item_wise"
    os.makedirs(MODEL_DIR, exist_ok=True)
    os.makedirs(RESULT_DIR, exist_ok=True)
    
    MODEL_FILE = f"{args.model}_{args.drop_rate}-{args.num_gradual}_{args.batch_size}@{args.best_k}.pth"
    args.model_path = os.path.join(MODEL_DIR, MODEL_FILE)
    assert not os.path.isdir(args.model_path), "Error: model path is a directory!"
    ...
    ## Prepare Datasets
    train_dataset = data_utils.NCF_UserWise_Dataset(user_num=user_num, item_num=item_num, features=train_data_list,train_mat=train_mat, true_labels=train_data_true_label, num_ng=args.num_ng)
    valid_dataset = data_utils.NCF_UserWise_Dataset(user_num=user_num, item_num=item_num, features=valid_data_list,train_mat=train_mat, true_labels=valid_data_true_label, num_ng=0)

    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn, collate_fn=custom_collate_fn)
    valid_loader = DataLoader(valid_dataset, batch_size=args.batch_size, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn, collate_fn=custom_collate_fn)
    ...
    results_df.to_csv(os.path.join(RESULT_DIR, f"{args.model}_{args.drop_rate}_{args.num_gradual}_{args.batch_size}_k{args.best_k}.csv"), index=False, float_format="%.4f")